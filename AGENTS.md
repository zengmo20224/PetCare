...
- AI 不得直接访问数据库，也不得提供疾病诊断、药物处方或治疗承诺。此处"数据库"指 **MySQL 业务库**——AI 访问业务数据必须通过受限 Tool → 业务 Service，不得依赖 Mapper/DataSource（由 `AiProviderArchitectureTest` 强制）。V2 引入的 PgVector 向量库属"派生知识索引"，AI 可读但只能存派生副本、不得作为业务真源，边界由 `docs/09-ai-agent-design.md` §2 B1/B6 约束。
