...
| P-001 | H5 正式公开登录采用手机号 + 密码，安全问题找回（已实现注册/登录/找回密码端点与 H5 页面；`@Profile("test")` 测试登录仅自动化测试可用） | 已决定 | 2026-06（原未决区首项） |
| P-002 | 微信小程序推进 **demo 形态**（非上架）：三态登录 Provider + 钱包支付复用 + 小程序兼容回归，可在微信开发者工具完整演示；真实上架资质流程仍延后 | 已决定（demo 落地 2026-08-01），详见下文专节 | 2026-08-01 |
| P-003 | 不申请微信支付商户号；长期规划，随 P-002 一并降级——demo 场景支付走钱包余额（D-010/D-012） | 已决定（降级） | 2026-07-04 |
...
## P-002 微信小程序 demo 形态（原未决区，2026-08-01 已决定）

状态：**已决定推进 demo 形态（非上架）**。原 2026-07-04 降级为"长期规划"的结论针对的是"真实对大陆用户营业"场景；本次用户将项目定位为"展示 AI coding 实力的 demo"，不追求上架，故资质/备案/微信支付不再是阻断——小程序端全面铺开到"可在微信开发者工具里完整演示"的程度。
...
当前**无处于未决状态的事项**。唯一延后的技术子项：

- **M8.4 / Q-4**：图片 NSFW 审核（自托管 ONNX vs 商用 API）——随 M8.4 独立切片再启，不阻塞任何当前工作，详见 D-013 未决子项与 `docs/09` §14。
